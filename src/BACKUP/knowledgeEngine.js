...
const detectProducts = (text) => {
  ...
  for (const product of PRODUCTS) {
    ...
    score += overlapScore(normalized, product.searchBlob, "en") * 8;
...
const renderSectionAnswer = (product, intent, lang) => {
  ...
  if (field === "features") {
    return `${product.name} comes with:\n${value}`;
  }

  if (field === "applications") {
    return `${product.name} is commonly used for:\n${value}`;
  }

  return `For ${product.name}, ${locale[field] || field} includes: ${value}`;
};
...
const buildSmartLocalReply = ({ text, lang, history = [], currentProductId = null }) => {
  const locale = T[lang] || T.en;
  const productMatches = detectProducts(text);
...
const getAssistantReply = async ({ text, history = [], currentProductId = null }) => {
