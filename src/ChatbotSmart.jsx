...
const getSafeSpeechRecognitionLang = (preferred) => {
  if (preferred === "mr-IN") return "hi-IN";
  if (preferred === "hi-IN") return "hi-IN";
  return "en-IN";
};

const getVoiceCandidates = (lang) => {
  if (lang === "mr") return ["mr-IN", "mr", "hi-IN", "hi", "en-IN", "en"];
  if (lang === "hi") return ["hi-IN", "hi", "mr-IN", "mr", "en-IN", "en"];
  return ["en-IN", "en-GB", "en-US", "en", "hi-IN", "mr-IN"];
};
...
export default function Chatbot({ chatOpen, setChatOpen }) {
  ...
  const voiceLangHintRef = useRef(null);
  ...
  const chooseVoice = useCallback((lang) => {
    ...
    for (const candidate of candidates) {
      const exact = voices.find(
        (voice) => normalized(voice.lang) === candidate.toLowerCase()
      );
    ...
    for (const candidate of candidates) {
      ...
      const partial = voices.find((voice) =>
        normalized(voice.lang).startsWith(base)
      );
  ...
  const normalizeForSpeech = useCallback((text) => {
    return (text || "")
      .replace(/[•📧📞]/g, " ")
      .replace(/\bPOS\b/g, " पी ओ एस ")
      .replace(/\bAEPS\b/g, " ए ई पी एस ")
      .replace(/\beKYC\b/gi, " ई के वाय सी ")
      .replace(/\bAadhaar\b/gi, " आधार ")
      .replace(/\bGPS\b/g, " जी पी एस ")
      .replace(/\bUPI\b/g, " यू पी आई ")
      .replace(/\s+/g, " ")
      .trim();
  }, []);
  ...
  const speak = useCallback(
    (text, lang, messageId) => {
      ...
      const utterance = new SpeechSynthesisUtterance(
        normalizeForSpeech(cleanBotText(text))
      );
    ...
    [chooseVoice, normalizeForSpeech, unlockAudio]
  );
  ...
  const finalizeListening = useCallback((submit = true) => {
    ...
    const langHint = voiceLangHintRef.current;

    transcriptRef.current = "";
    lastPreviewRef.current = "";
    heardSpeechRef.current = false;
    voiceLangHintRef.current = null;

    setIsListening(false);
    setInput("");

    if (submit && finalText) {
      void sendRef.current?.(finalText, { langHint });
    }
  }, []);
  ...
  const startListen = useCallback(() => {
    ...
    const recognitionLang = getSafeSpeechRecognitionLang(micLang);

    transcriptRef.current = "";
    lastPreviewRef.current = "";
    heardSpeechRef.current = false;

    voiceLangHintRef.current =
      micLang === "mr-IN" ? "mr" : micLang === "hi-IN" ? "hi" : "en";
    ...
    const resetSilenceTimer = (heardSpeech = heardSpeechRef.current) => {
      clearTimeout(silenceTimeoutRef.current);
      silenceTimeoutRef.current = setTimeout(() => {
        try {
          recognition.stop();
        } catch {}
      }, heardSpeech ? 2200 : 6500);
    };
    ...
    recognition.onresult = (event) => {
      ...
      if (finalChunk || interimChunk) heardSpeechRef.current = true;
    ...
    recognition.onerror = (event) => {
      ...
      if (event?.error === "language-not-supported") {
        alert("Marathi mic is not supported directly in this browser. Hindi recognition fallback is being used.");
      }
    };
    ...
    try {
      ...
      setIsListening(false);
      alert("Could not start microphone. Please allow mic access and try Chrome or Edge.");
    }
  }, [finalizeListening, micLang, stopSpeak, unlockAudio]);
  ...
  const toggleListening = useCallback(() => {
    ...
    startListen();
  }, [isListening, isSpeaking, startListen, stopListen, stopSpeak]);
  ...
  const send = useCallback(async (text, options = {}) => {
    ...
    const lang = options.langHint || detectLang(trimmed);
    ...
    const currentProductId =
      [...history].reverse().find((message) => message.productId)?.productId || null;
    ...
    try {
      const reply = await getAssistantReply({
        ...
        history,
        currentProductId,
        langHint: options.langHint || null,
      });

      console.log("CHATBOT_REPLY_MODE", reply);
      ...
      setMsgs((previous) => [
        ...previous,
        {
          id: Date.now() + 1,
          role: "bot",
          text: cleanBotText(reply.answer),
          ...
          >
            ...
            {!isListening && !typing && (
              <div style={{ textAlign: "center", marginTop: 6, fontSize: 10.5, color: MUTED }}>
                {audioUnlocked
                  ? "Voice replies are ready."
                  : voicesReady
                  ? "Tap the text box or mic once to enable voice replies."
                  : "Loading voice support..."}
              </div>
            )}
          </div>
        </div>
